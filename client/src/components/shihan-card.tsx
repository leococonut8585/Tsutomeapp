...
export function ShihanCard({ shihan, onComplete, onClick }: ShihanCardProps) {
  const targetDate = new Date(shihan.targetDate);
  const daysRemaining = differenceInDays(targetDate, new Date());
  const progress = shihan.completed ? 100 : Math.max(0, Math.min(100, ((30 - daysRemaining) / 30) * 100));
        ...
        <div className="flex-1 min-w-0 space-y-3">
          ...
          {!shihan.completed && onComplete && (
            <Button
              ...
              className="w-full mt-2 border-chart-3/50 hover:bg-chart-3/10"
